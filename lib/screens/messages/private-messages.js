...
import { useChatMessages, useDebouncedState, useUserInfo } from 'hooks'
...
const PrivateMessages: (args: any) => React$Node = ({
  ...
  const markAsRead = useCallback(() => {
    debugLog('[MESSAGES] markAsRead')

    chat.read = userUid
  }, [chat, userUid])

  useEffect(() => {
    debugLog('[MESSAGES] useEffect refreshChat')
  ...
  useEffect(() => {
    const thisChatUid = message.chatUid || chatUid

    if (thisChatUid) {
      debugLog('[MESSAGES] useEffect subscribeToChat', thisChatUid)
  ...
  const onSend = useCallback(
    async (messagesToSend = []) => {
      ...
      chat.lastMessage = { lastMessage: currentMessage.text, userUid }
    },
