...
import { getLocation, hasLocationPermission } from 'utils/setup'
...
const SelectLocationMapScreen: (args: any) => React$Node = ({
  ...
  const clearLocation = useCallback(() => onLocationChange(locationSchema), [
    onLocationChange,
  ])

  const useCurrent = useCallback(() => {
    setCurrentLocation({ forceUpdate: true, animateToRegion: true })
  }, [setCurrentLocation])

  const checkPermission = useCallback(async () => {
    setHasPermission(await hasLocationPermission())
  }, [])
