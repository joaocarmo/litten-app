...
import { useCrashlytics } from 'hooks'
...
const ProfileSettingsScreen: (args: any) => React$Node = ({
  ...
  const hasPhoneNumber = () => {
    if (!user.phoneNumber) {
      Alert.alert(translate('feedback.errorMessages.noPhoneNumber'))
  ...
  const renderItem = ({ item: { title, description, value, setter } }) => (
    <UISwitch description={description} value={value} onValueChange={setter}>
      {title}
    </UISwitch>
  )
