...
import { useSearchFilters, useSearchFiltersSpecies, useUnit } from 'hooks'
import {
...
const HomeFilterSetScreen: (args: any) => React$Node = ({ filter, title }) => {
  ...
  const removeType = useCallback((type) => dispatch(removeTypeAction(type)), [
    dispatch,
  ])
  ...
  const setType = useCallback((type) => dispatch(setTypeAction(type)), [
    dispatch,
  ])

  const setUserType = useCallback(
    (userType) => dispatch(setUserTypeAction(userType)),
    [dispatch],
  )
