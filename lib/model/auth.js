...
import { DB_AUTH_USER_KEY_ID, DB_USER_KEY_ID } from 'utils/constants'
...
export default class Auth implements AuthClass {
  ...
  async signIn() {
    if (this.#email && this.#password) {
      ...
      this.#userAuthUid = this.auth.currentUser.uid
      await this.getUser()
    } else {
  ...
  async createUser() {
    if (this.#email && this.#password) {
      ...
      await createNewUser({
        [DB_USER_KEY_ID]: this.#userAuthUid,
        [DB_AUTH_USER_KEY_ID]: this.#userAuthUid,
  ...
  async getUser() {
    if (this.#userAuthUid) {
      return getUserFromUid(this.#userAuthUid)
    }
    return null
  }
