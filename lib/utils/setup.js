...
import { DEBOUNCE_TIMEOUT, RECURSION_LIMIT } from 'utils/constants'
...
export const getCurrentPosition = (): Promise<{ coords: any }> => {
...
export const getLocation = async (): Promise<{
  ...
  const emptyResponse = { coords: { latitude: null, longitude: null } }
  ...
  try {
    return await execOrTimeout(getCurrentPosition(), DEBOUNCE_TIMEOUT)
  } catch (err) {
    logError(err)
  }
