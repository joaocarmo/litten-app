...
import { WEB_APP_BASE } from 'utils/env'
import { littenSpeciesList, littenTypes } from 'utils/litten'
import { translate } from 'utils/i18n'
import { setTimeout } from '../../node_modules/react-native/Libraries/Core/Timers/JSTimers'
 ...
 * @returns {number[]}
 */
export const iterateTimes = (n: number, offset: number = 1): number[] => {
  if (n < 0) {
    return []
  }
...
export const mapCountriesToSelector = (item: {
  cca2: string,
  name: { common: string },
}): void | { key: string, label: string, value: string } => {
