  ...
  if (
    ...
    for (const attachment of attachments) {
      ...
      try {
        ...
        if (response.ok) {
          ...
          logError(uploadUrl, headersUpload, response)
        }
      } catch (err) {
        logError(err)
      }
    ...
    if (temporaryAttachmentIds.length > 0) {
      ...
      try {
        ...
        if (!response.ok) {
          logError(appendUrl, bodyAppend, response)
        }
      } catch (err) {
        logError(err)
      }
...
export const submitUserFeedback = async (
  ...
  if (url && postData) {
    ...
    if (response.ok) {
      ...
      logError(url, postData, response)
    }
  }
...
/**
 * Queries the website for the app maintenance status
 * @async
 * @returns {boolean}
 */
export const getMaintenanceMode = async () => {
  const appSettingsUri = `${WEB_APP_BASE}/.well-known/app.json`

  try {
    const data = await fetcher(appSettingsUri)
    const jsonData = await data.json()
    return jsonData?.maintenanceMode ?? false
  } catch (err) {
    handleNetworkError(err)
  }
