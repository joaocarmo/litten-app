...
const LittenPostScreen: (args: any) => React$Node = ({
  ...
  const user = new User(route?.params?.user)
  const litten = new Litten(route?.params?.litten)
  ...
  const createdAt =
    (litten.metadata?.createdAt?._seconds ?? 0) * 1000 || undefined
      ...
      <View style={styles.littenPostFooter}>
        <View style={styles.littenPostFooterUser}>
          ...
          <View style={styles.littenPostFooterUserInfo}>
            ...
            <UIText noPadding>{dayjs(createdAt).fromNow()}</UIText>
          </View>
        </View>
        {typeLabel && (
