...
const HomeFilter: (args: any) => React$Node = (props) => (
  <ScreenTemplate
    header={
...
const HomeFilterSet: (args: any) => React$Node = ({ route }) => (
  <ScreenTemplate
    header={
      <ScreenSimpleHeaderTemplate withGoBack>
        {route?.params?.title}
    ...
    scrollable={route?.params?.filter !== LITTEN_FILTER_LOCATION_RADIUS}>
    <FilterSet filter={route?.params?.filter} />
  </ScreenTemplate>
)

const HomeScreen: (args: any) => React$Node = (props) => (
