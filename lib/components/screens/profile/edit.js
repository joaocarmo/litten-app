...
const ProfileEditScreen: (args: any) => React$Node = ({
  ...
  const saveChanges = () => {
    ...
    if (hasChanges) {
      user.displayName = editedDisplayName ?? displayName
      user.isOrganization = editedIsOrganization ?? isOrganization
      setDisplayName(editedDisplayName ?? displayName)
      handleSetEmail(editedEmail ?? email)
      setIsOrganization(editedIsOrganization ?? isOrganization)
  ...
  return (
    <Pressable onPress={Keyboard.dismiss} style={styles.editContainer}>
      ...
      <View style={styles.editPhoto}>
        <SelectPhoto
          photoURL={editedPhotoURL ?? photoURL}
          onChange={setEditedPhotoURL}
        />
      </View>
