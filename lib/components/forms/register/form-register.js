...
import { getErrorMessage, parseAvatar, logError } from 'utils/functions'
...
const RegisterForm: (args: any) => React$Node = (props) => {
  ...
  const submitForm = async () => {
    try {
      ...
      const authUuid = auth().currentUser.uid
      const downloadURL = await uploadUserAvatar(avatar, { authUuid })
      await auth().currentUser.updateProfile({
        displayName,
        photoURL: parseAvatar((downloadURL: any), { email }),
      })
      await createNewUser({
        authUuid,
        country,
