...
describe('Test the "filterData" function', () => {
  ...
  const dataTable = [
    ...
    [
      [litten],
      { filters: { ...filters, littenType: [LITTEN_TYPE_BREED] } },
      [],
    ],
...
describe('Test the "execOrTimeout" function', () => {
  ...
  it.skip('Fails to execute a promise before the timeout limit', async () => {
    jest.useRealTimers()

    expect.assertions(1)

    const returnValue = 'there'
    const errorMessage = 'Async function timed out'
