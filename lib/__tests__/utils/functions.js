...
describe('Test the "appendOrReplace" function', () => {
  ...
  it('returns a concatenated string of 2 inputs with a prefix on the second', () => {
    expect(appendOrReplace('string1', 'string2', { prefix: 'prefix' })).toBe(
      'string1prefixstring2',
    )
  })

  it('returns a concatenated string of 2 inputs with a suffix on the second', () => {
    expect(appendOrReplace('string1', 'string2', { suffix: 'suffix' })).toBe(
...
describe('Test the "filterData" function', () => {
  ...
  const dataTable = [
    [[], { filters }, []],
    [[litten], { filters }, [litten]],
    [
      [litten],
      { filters: { ...filters, littenSpecies: [LITTEN_SPECIES_DOG] } },
      [],
    ],
    [[litten], { filters: { ...filters, littenType: [LITTEN_TYPE_LOST] } }, []],
    [[litten], { filters: { ...filters, locationRadius: 50 } }, []],
    [
      [litten],
